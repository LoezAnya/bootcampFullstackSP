...
public class TransactionController {
    ...
    public ResponseEntity<Transaction> createTransaction(@RequestBody Transaction transaction) {
        ...
        if (transaction.getTransaction_type().toLowerCase().equals("consignacion")) {
            sender = accountService.getAccountByNumber(transaction.getId_sender_account()).map(account -> {
            ...
            }).orElseThrow(() -> new RuntimeException("Not found" + transaction.getId_sender_account()));
        }
        if (transaction.getTransaction_type().toLowerCase().equals("retiro")) {

            Optional<Account> opcAccount = accountService.getAccountByNumber(transaction.getId_sender_account());
            if (opcAccount.isPresent()) {
                Account account = opcAccount.get();
                if (account.getAccount_state().equals(Account.State.INACTIVE)) {
        ...
        if (transaction.getTransaction_type().toLowerCase().equals("transferencia")) {
            sender = transaction;
            Optional<Account> accountSender = accountService.getAccountByNumber(transaction.getId_sender_account());
            if (accountSender.isPresent()) {
                Account accountSen = accountSender.get();
                if (accountSen.getAccount_state().equals(Account.State.INACTIVE)) {
                    return new ResponseEntity<>(HttpStatus.NOT_ACCEPTABLE);
                } else {
                    ...
                    Optional<Account> accountReceptor = accountService.getAccountByNumber(transaction.getId_receptor_account());
